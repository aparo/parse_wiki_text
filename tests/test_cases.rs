...
fn parse_cases(content: &str) -> Vec<Case> {
  content
    ...
    .map(|case| {
      ...
      let nodes = nodes.strip_prefix("node:").expect("expected node:").trim();

      let warnings = options.next().expect("expected warn:");
      let warnings = warnings.strip_prefix("warn:").expect("expected warn:").trim();

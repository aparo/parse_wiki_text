use parse_wiki_text::{DefinitionListItem, ListItem, Node, Parameter, TableCaption, TableCell, TableRow, Warning};
...
{
  fn to_test_str(&self) -> String {
    self.as_ref().map(|t| t.to_test_str()).unwrap_or(String::new())
  }
}
...
impl ToTestStr for Node<'_> {
  fn to_test_str(&self) -> String {
    ...
    match self {
      ...
      Parameter { default, name, .. } => format!("Parameter({}, {})", default.to_test_str(), name.to_test_str()),
      Preformatted { nodes, .. } => {
      ...
      Template { name, parameters, .. } => format!("Template({}, {})", name.to_test_str(), parameters.to_test_str()),
...
impl ToTestStr for DefinitionListItem<'_> {
  fn to_test_str(&self) -> String {
    format!("DefinitionListItem({:?}, {})", self.type_, self.nodes.to_test_str())
  }
}
...
impl ToTestStr for Parameter<'_> {
  fn to_test_str(&self) -> String {
    format!("Parameter({}, {})", self.name.to_test_str(), self.value.to_test_str())
  }
}

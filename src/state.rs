...
impl<'a> State<'a> {
  pub fn flush(&mut self, end_position: usize) {
    flush(&mut self.nodes, self.flushed_position, end_position, self.wiki_text);
  }
...
pub fn flush<'a>(nodes: &mut Vec<crate::Node<'a>>, flushed_position: usize, end_position: usize, wiki_text: &'a str) {
...
pub fn skip_whitespace_backwards(wiki_text: &str, mut position: usize) -> usize {
  while position > 0 && wiki_text.as_bytes()[position - 1].is_ascii_whitespace() {
    position -= 1;
  }
  position
}
...
pub fn skip_whitespace_forwards(wiki_text: &str, mut position: usize) -> usize {
  ...
  while wiki_text.get(position).map_or(false, |c| c.is_ascii_whitespace()) {
    position += 1;
  }
  position
}

...
pub fn parse_heading_cell(state: &mut crate::State) {
  ...
  if let crate::state::TableState::HeadingFirstLine = table.state {
    ...
    crate::state::flush(&mut state.nodes, state.flushed_position, end, state.wiki_text);
...
fn change_state(
  ...
  match table.state {
    TableState::Before => {
      ...
      table.before.append(&mut std::mem::replace(&mut state.nodes, vec![]));
    }
...
fn parse_line_break(
...
) {
  {
    let table = get_table(&mut state.stack);
    match table.state {
      TableState::Before | TableState::CaptionRemainder => state.scan_position = position_after_token,
      TableState::CaptionFirstLine => {
      ...
      TableState::CellFirstLine => {
        crate::state::flush(
          ...
          crate::state::skip_whitespace_backwards(state.wiki_text, position_before_line_break),
          state.wiki_text,
        );
      ...
      TableState::CellRemainder | TableState::HeadingRemainder => state.scan_position = position_before_line_break + 1,
      TableState::TableAttributes => {
        crate::state::flush(
          &mut state.nodes,
          state.flushed_position,
...
pub fn parse_inline_token(state: &mut crate::State) {
  let table = get_table(&mut state.stack);
  let position_before_token = state.scan_position;
  if state.wiki_text.as_bytes().get(position_before_token + 1).cloned() == Some(b'|') {
    match table.state {

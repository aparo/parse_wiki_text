...
pub fn parse_comment(state: &mut crate::State) {
  ...
  while let Some(character) = state.get_byte(position) {
    match character {
      b'-' if state.get_byte(position + 1) == Some(b'-') && state.get_byte(position + 2) == Some(b'>') => {
        position += 3;
        break;
      }
...
fn parse_end_tag(state: &mut crate::State, comment_start_position: usize, tag_start_position: usize) -> bool {

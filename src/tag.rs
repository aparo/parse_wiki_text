        ...
        match tag_class {
          crate::TagClass::ExtensionTag => {
            if state.get_byte(tag_end_position - 2) == Some(b'/') {
              ...
              match &tag_name as _ {
                "math" | "nowiki" => {
                  parse_plain_text_tag(state, start_position, tag_end_position, &tag_name);
                }
                _ => {
                  state.push_open_node(crate::OpenNodeType::Tag { name: tag_name }, tag_end_position);
                }
              }
            }
          }
...
fn parse_plain_text_end_tag<'a>(
  ...
  let position_after_end_tag = loop {
    match state.get_byte(position_after_end_tag_name) {
      ...
      Some(b'\t') | Some(b'\n') | Some(b' ') => {
        let position_after_end_tag = state.skip_whitespace_forwards(position_after_end_tag_name + 1);

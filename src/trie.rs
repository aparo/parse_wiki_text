      ...
      'a: while let Some(byte) = byte_item {
        ...
        if byte_item.is_none() {
          if character_item.is_none() {
            ...
            if case_folded {
              self.add_folded_characters(character, state_before_character, State::Final(payload));
            }
            return true;
          }
          ...
          self.states.push(vec![]);
          if case_folded {
            self.add_folded_characters(character, state_before_character, State::Continue(next_state));
          }

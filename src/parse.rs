...
#[must_use]
pub fn parse<'a>(configuration: &crate::Configuration, wiki_text: &'a str) -> Result<crate::Output<'a>, Error> {
  ...
  loop {
    ...
    match state.get_byte(state.scan_position) {
      ...
      Some(0) | Some(1) | Some(2) | Some(3) | Some(4) | Some(5) | Some(6) | Some(7) | Some(8) | Some(11) | Some(12)
      | Some(13) | Some(14) | Some(15) | Some(16) | Some(17) | Some(18) | Some(19) | Some(20) | Some(21) | Some(22)
      | Some(23) | Some(24) | Some(25) | Some(26) | Some(27) | Some(28) | Some(29) | Some(30) | Some(31)
      | Some(127) => {
        state.warnings.push(crate::Warning {
      ...
      Some(b'&') => crate::character_entity::parse_character_entity(&mut state, configuration),
      Some(b'\'') => {
        if state.get_byte(state.scan_position + 1) == Some(b'\'') {
      ...
      Some(b']') => match state.stack.pop() {
        ...
        Some(crate::OpenNode {
          ...
          if state.get_byte(state.scan_position + 1) == Some(b']') {
            crate::link::parse_link_end(&mut state, &configuration, start, nodes, namespace, target);
          } else {
            state.scan_position += 1;

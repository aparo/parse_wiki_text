  ...
  if stack_length > 0 {
    ...
    } = &mut state.stack[stack_length - 1]
    {
      ...
      if name.is_none() {
        ...
        state.flushed_position = crate::state::skip_whitespace_forwards(state.wiki_text, state.scan_position + 1);
...
pub fn parse_parameter_separator(state: &mut crate::State) {
  match state.stack.last_mut() {
    Some(crate::OpenNode {
      ...
      if name.is_none() {
        let position = crate::state::skip_whitespace_backwards(state.wiki_text, state.scan_position);
        crate::state::flush(&mut state.nodes, state.flushed_position, position, state.wiki_text);
        *name = Some(std::mem::replace(&mut state.nodes, vec![]));
      } else {
        crate::state::flush(
          &mut state.nodes,
  ...
  match state.stack.last() {
    ...
    Some(crate::OpenNode {
      ...
      Some(crate::OpenNode {
        nodes,
        start,
        type_: crate::OpenNodeType::Template { name, mut parameters },
      }) => {
    ...
    _ => {
      if state.stack.iter().rev().skip(1).any(|item| match item.type_ {
        crate::OpenNodeType::Parameter { .. } => state.get_byte(state.scan_position + 2) == Some(b'}'),
